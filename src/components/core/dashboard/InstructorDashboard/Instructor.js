...
import { getInstructorDashboardData } from "../../../../services/operations/userAPI";
...
const Instructor = () => {
  ...
  useEffect(() => {
    const getinstructorDashBoardData = async () => {
      setLoading(true);
      const instructordata = await fetchInstructorCourses(token);
          ...
          <div className="w-full mt-3">
            <div className="flex flex-col lg:flex-row items-start gap-3 justify-between w-full">
              {totalAmount > 0 || totalStudent > 0 ? (
                <InstructorCart courses={instructorData}></InstructorCart>
              ) : (
              ...
              <div className="flex w-full lg:min-w-[250px] flex-col rounded-md bg-richblack-800 p-6">
            ...
            {/* Courses */}
            <div className="rounded-md bg-richblack-800 p-6 mt-3">
              ...
              <div className="my-4 flex flex-col md:flex-row items-center space-x-0 md:space-x-6 gap-3">
