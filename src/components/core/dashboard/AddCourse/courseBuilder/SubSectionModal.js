...
const SubSectionModal = ({
  ...
  const handelEditSubSection = async () => {
    ...
    if (currentValues.lectureVideo && currentValues.lectureVideo instanceof File) {
      formData.append("videoFile", currentValues.lectureVideo);  // <-- FIXED
    }
  ...
  const onSubmitHandler = async (data) => {
    ...
    formData.append("videoFile", data.lectureVideo); // <-- CRITICAL FIX
  ...
  return (
    <div className="fixed inset-0 z-[1000] grid h-screen w-screen place-items-center overflow-auto bg-white bg-opacity-10 backdrop-blur-sm">
      <div className="my-10 w-11/12 max-w-[700px] rounded-lg border border-richblack-400 bg-richblack-800">
        ...
        {/* ---------------- FORM ---------------- */}
        <form className="flex flex-col gap-5 px-8 py-10" onSubmit={handleSubmit(onSubmitHandler)}>
          ...
          {/* Title */}
          <label>
            <p className="mb-1 text-sm text-richblack-5">Lecture Title</p>
            <input
              disabled={view || loading}
              placeholder="Enter Lecture Title"
              {...register("lectureTitle", { required: true })}
              className="w-full rounded bg-richblack-700 p-3 text-richblack-5"
            />
            {errors.lectureTitle && (
              <span className="text-xs text-yellow-100">Title is required</span>
            )}
          </label>

          {/* Description */}
          <label>
            <p className="mb-1 text-sm text-richblack-5">Lecture Description</p>
            <textarea
              disabled={view || loading}
              rows="3"
              placeholder="Enter Lecture Description"
            ...
            />
            {errors.lectureDesc && (
              <span className="text-xs text-yellow-100">Description is required</span>
            )}
          </label>
