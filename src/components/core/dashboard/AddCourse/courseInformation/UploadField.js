...
useEffect(() => {
  ...
  useEffect(() => {
    if (selectedFile) {
      setValue(name, selectedFile); // MUST SEND File object
    }
  }, [selectedFile]);
  ...
  return (
    <label>
      ...
      <div
        className={`${
          isDragActive ? "bg-richblack-600" : "bg-richblack-700"
        } flex min-h-[220px] items-center justify-center rounded-md border-2 border-dotted border-richblack-500 cursor-pointer`}
      >
        {previewSource ? (
      ...
      {errors[name] && (
        <span className="text-[12px] text-yellow-100">{label} is required</span>
      )}
    </label>
  );
};
