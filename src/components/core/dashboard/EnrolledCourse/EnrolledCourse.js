...
const EnrolledCourse = () => {
  ...
  // -------------------------------------------------
  // SAFELY NAVIGATE TO COURSE OR FALLBACK
  // -------------------------------------------------
  const goToCourse = (course) => {
    const section = course?.courseContent?.[0];
    const subSection = section?.subSection?.[0];

    // If no sections or subsections → go to course overview page
    if (!section || !subSection) {
      console.warn("No sections/subsections found, redirecting to course root.");
